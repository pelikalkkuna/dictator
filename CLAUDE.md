...
- **Pelikoodi:** kohdat 1–7 valmiit — runko, kassajärjestelmä, ryhmät/mittarit, audienssijärjestelmä (42 korttia), presidentin päätökset (19 korttia), uutisvaihe (48 korttia) ja sota (GDD 8. luku: A1-pikasota, P1/D16-eskalaatio N1-kierteen todennäköisyystaulukolla, voimalaskenta, voiton jälkitila Leftoton puolitus + REV STR -piikki, sotavelka, tappio → peli päättyy -tila) toimivat selaimessa. "Seuraava kuukausi" -nappi testauksen ajaksi, korvataan varsinaisella kuukausikierroksella myöhemmässä vaiheessa.
- **Testaus:** `node --test testit/*.test.js` (tai `npm test`) — 52 testiä kattaa kassaraportin, mittarien rajauksen, audienssit, presidentin päätökset, uutisvaiheen ja sodan (GDD 8.3:n molemmat esimerkit, pikasota, N1-eskalaation kaikki haarat, sotavelka, REV STR -paluu), kaikki vihreää
- **Seuraava askel:** kohta 8 toteutusjärjestyksestä — vallankumous/kaappaus/kapina + puolustusvalinta
...
- **KYSYTTÄVÄ SASULTA — REV STR:n paluukäyrä:** GDD 8.4 sanoo "REV STR piikkaa Ritimban sotavoiman tasolle ja palautuu 3 kk:ssa" mutta ei anna kaavaa. Koodissa (`js/moottori/sota.js`, `kasitteleVallankumousvoimanPalautuminen`) toteutettu tasaisena lineaarisena paluuna kolmen kuukauden yli (esim. piikki 19 → kolme kk myöhemmin tasan 10). Ei vielä kytketty mihinkään pelimekaniikkaan koska vallankumous/kaappaus/kapina (kohta 8) käyttää vallankumousvoimaa mutta ei ole vielä rakennettu.
- **Huom — pelin lopetustila:** Sodan hävitessä `pelitila.peliOhi` menee todeksi ja "Seuraava kuukausi" -nappi lukkiutuu pysyvästi. Tämä on vasta minimaalinen paikkamerkki — varsinainen likvidaatio/pako/pisteytys-näkymä (GDD 11: Pako, 14: Pisteytys) rakennetaan kohdassa 10.
