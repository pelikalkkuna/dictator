...
- **Testaus:** `node --test testit/*.test.js` (tai `npm test`) — 109 testiä kattaa kassaraportin, mittarien rajauksen, audienssit, presidentin päätökset, uutisvaiheen, sodan, kriisit, attentaatin, paon ja pisteytyksen, kaikki vihreää. Swipe-eleet testattu selaimessa (hiiri, aito kosketus, snap-back) mutta ei Node-testeissä (UI-vuorovaikutusta, ei sääntölogiikkaa).
...
- **Seuraava askel:** ei enää yksittäistä "seuraavaa kohtaa" eikä avoimia kysymyksiä. Jäljellä: poliisiraportti + täysi 8-vaiheinen kuukausikierros (nykyinen "Seuraava kuukausi" on yhä yksinkertaistettu), sekä Sasun oma pelitestaus selaimessa nyt kun kaikki mekaniikat ja niiden epäselvät kohdat ovat ratkottu.
