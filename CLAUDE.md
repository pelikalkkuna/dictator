...
- **Pelikoodi:** kohdat 1–6 valmiit — runko, kassajärjestelmä, ryhmät/mittarit, audienssijärjestelmä (42 korttia), presidentin päätökset (19 korttia) ja uutisvaihe (kaikki 48 korttia GDD 7.1–7.4:stä: yhteinen pakka jossa kertakäyttöiset ja toistuvat/ehdolliset kortit, N43–N48:n ehtotarkistus nostohetkellä, pakan uudelleentäyttö) toimivat selaimessa. "Seuraava kuukausi" -nappi testauksen ajaksi, korvataan varsinaisella kuukausikierroksella myöhemmässä vaiheessa.
- **Testaus:** `node --test testit/*.test.js` (tai `npm test`) — 39 testiä kattaa kassaraportin, mittarien rajauksen, audienssit, presidentin päätökset ja uutisvaiheen (kortin nosto, ehdollisten korttien uudelleenveto, pakan täyttyminen), kaikki vihreää
- **Seuraava askel:** kohta 7 toteutusjärjestyksestä — sota ja N1-eskalaatiokierre (tämä ratkaisee myös uutiskorttien N1/N3 vielä toteuttamattoman logiikan)
...
- **KYSYTTÄVÄ SASULTA — uutiskortit ilman lukuarvoa:** N4 "Armeijan asevarasto räjähti" → "Armeijan voima romahtaa" ja N5 "Banaanien hinta romahtaa" → "Kassatulot laskevat" / "−tuloja" eivät sisällä GDD:ssä mitään numeroa. Koodissa (`js/data/uutiset.js`, `erikoinen: "N4_EI_MAARITELTY"` / `"N5_EI_MAARITELTY"`) ne kuluvat pakasta mutta EIVÄT TEE MITÄÄN vielä. Pitää saada tarkka luku/kaava Sasulta.
...
- **Keskeneräistä (odottaa kohtaa 7, Sota):** Uutiskortit N1 (Leftoton sotauhka), N3 (Leftoto hyökkää!) kuluvat/vaikuttavat vain osittain — N1:n suosiovaikutus toimii mutta "voi laukaista N3:n" -eskalaatiokytkentä ja N3:n koko sodan ratkaisu (GDD 8. luku: N1-eskalaatiokierre, voimalaskenta, velkavaihe) puuttuvat kokonaan. Sama koskee audienssikortti A1:n *PIKASOTA ja P1/D16:n *ESKALAATIO-merkintöjä — ne soveltavat vain suosio/voima/talous-osuutensa, eivät laukaise mitään sotaa.
