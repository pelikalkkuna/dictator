...
- **Seuraava askel:** pelitestikierrokset 1 ja 2 tehty ja palautteet toteutettu (ks. osiot alla). Peli on julkaistu ja pelattavissa. Seuraavaksi tapahtumakuvien tuottaminen ja lisää pelitestausta.
...
## Pelitestipalaute 2 (Sasu, elokuu 2026) — korjattu

- **KORJATTU BUGI — kuukausi jumittui vaiheeseen 6, "Jatka" harmaana.** Poliisiraportti on ainoa vaihe joka esiintyy kuukaudessa kahdesti (vaiheet 2 ja 6), mutta paneeleita on vain yksi ja se sijaitsi DOM:issa kuukauden alussa. Koska paneelit jäävät näkyviin kuun loppuun, vaiheen 6 raportti piirtyi uutisten YLÄPUOLELLE: mitattuna ainoa toimiva nappi oli 892 px ylempänä sivulla, eli 844 px korkealla puhelimella kokonaan ruudun ulkopuolella. Pelaaja näki pohjalla vain lukossa olevan "Jatka"-napin. Korjattu `siirraPoliisiraporttiVaiheeseen`-funktiolla (`piirto.js`), joka siirtää paneelin DOM:issa siihen kohtaan jossa se kuukauden kulussa on. Mittaus korjauksen jälkeen: 52 px.
- **Poliisiraportti tarjotaan enää kerran kuussa, viimeisenä toimintona** (Sasu: "Muutetaan niin että poliisiraportti on ostettavissa kk viimeinen toiminto. Jätetään se kassan jälkeen tapahtuva rapsan osto väliin. Ei tarjota kahta kertaa kuussa."). Kuukausi on nyt 5-vaiheinen: kassaraportti → audienssi → päätös → uutiset → poliisiraportti. Samalla edellinen DOM-siirtokorjaus kävi tarpeettomaksi ja poistettiin — paneeli on nyt yksinkertaisesti oikeassa paikassa HTML:ssä. **Hyvä sivuvaikutus:** raportti ostetaan nyt SEURAAVAA kuukautta varten, joten sen tieto on pelaajan muistin varassa kun päätökset tehdään — juuri kuten GDD 9.5:n puolustusvalinta edellyttää. Raportti tyhjenee seuraavan kuun alussa.
- **N3:n yllätyshyökkäys vaatii huonot välit.** Sasu: "Leftoto yllätyshyökkäys kortti vaatii myös 3 tai alle suosion että se realisoituu. Äksen oli 9 suosio niin ei sitä ny silloin hyökätä vaikka kortti tulee." Toteutettu N3:n `ehto`-kentällä (`leftoto.suosio <= 3`). **Vaati pienen moottorimuutoksen:** `ehto` tarkistettiin aiemmin vain TOISTUVILLA korteilla (N1, N43–N48), joten kertakäyttöisen N3:n ehto olisi jäänyt kokonaan huomiotta. Nyt `nostaUutinen` tarkistaa ehdon myös pakan korteilta, ja ehdon pettäessä kortti jää pakkaan kulumatta ja nostetaan seuraava — sama semantiikka kuin GDD 4.2 kuvaa. Koskee vain satunnaista nostoa: A1:n pikasota ja P1/D16:n eskalaatio ovat pelaajan omia tekoja eivätkä käy tätä kautta.
- **Lisätty automaattinen vieritys uuteen vaiheeseen** (`vieritaNakyviin`). Kuukausi kertyy ylhäältä alas, joten uusi vaihe voi jäädä taitteen alle myös muissa kohdissa; nyt se tuodaan näkyviin. Kunnioittaa `prefers-reduced-motion`-asetusta.

**Testausoppi — kolmas samaa sukua oleva bugi.** Playwrightin `page.click()` löytää ja klikkaa elementin riippumatta siitä missä se on sivulla, joten se EI paljasta "kontrolli on pelaajan ulottumattomissa" -vikoja. Soak-testit menivät läpi koko ajan. Aiemmat kaksi samaa perhettä: `setPointerCapture` vei napilta click-tapahtuman, ja `style.display=""` ei kumonnut CSS:n `display:none`-sääntöä. **Kun UI-vaiheita lisätään, mittaa myös SIJAINTI:** onko ainoa toimiva kontrolli sillä ruudulla jolla pelaaja on. `getBoundingClientRect()` + `window.scrollY` + `innerHeight` riittää.
...
- **Yhdistäminen mainiin: Claude tekee sen automaattisesti (Sasu, elokuu 2026: "Ala tekemään yhdistämisiä automaattisesti, aina kun on tarpeen"; täsmennetty myöhemmin: "Tämä ei ole tuotanto vs dev -tilanne vaan viimeisin tilanne saa olla aina Githubissa myös käytössä").** Kehitys tapahtuu kunkin tehtävän omassa haarassa (haaran nimi vaihtelee session mukaan, ei ole yksi pysyvä kehityshaara); kun kokonaisuus on valmis ja testit vihreinä, se yhdistetään mainiin fast-forwardilla ilman erillistä kysymistä, jolloin julkaisu lähtee heti. Tee siivoukset (esim. workflow-muutokset) kehityshaaraan ENNEN yhdistämistä, jotta yhdistäminen pysyy fast-forwardina eivätkä haarat lähde eriämään.
...
- **RATKAISTU (Sasu, elokuu 2026) — pisteytystitteli kuolemantapauksessa:** "Kuollut jää ilman mahdollisia pankkitilin rahoja, ne kun realisoituvat pisteiksi vasta jos selviää hengissä. Ei muuta eroa." Titteliraja-taulukko pysyy siis puhtaasti pistepohjaisena (GDD 14 kirjaimellisesti), ja kuoleman ainoa pisteseuraus on Swiss-bonuksen menetys. Nykytoteutus oli jo tämä (`laskeSwissBonus` palauttaa 0 ellei `pakeniHengissa`) — ei koodimuutosta. Kuolemaan päättynyt peli voi siis saada tittelin "Kunniakas pako", ja se on tarkoitettua: titteli mittaa valtakauden onnistumista, ei loppukohtausta.
...
- **UUSI MEKANIIKKA (Sasu, elokuu 2026) — audienssi on nelisuuntainen, GDD:n ulkopuolinen:** Sasun idea, tietoisesti toteutettu tässä vaiheessa "kun moottori on perustasolla eikä vielä ole mitään uutta lisätty" — muutamia muitakin nelisuuntaisen kortin variaatioita on jo mietitty myöhemmälle vaiheelle. Oikealle/vasemmalle = Kyllä/Ei kuten ennen. **Ylös = "Ehdota jotain muuta"**, sallittu kerran per audienssi: delegaatio nostaa toisen kortin OMASTA pakastaan, alkuperäinen kortti ei kulu (voi tulla vastaan myöhemmin). Toinen ehdotusyritys samassa audienssissa — tai jo ensimmäinen jos ryhmän pakassa ei ole muuta tarjottavaa — ohjautuu automaattisesti Skipiksi ("delegaatio tuumaa että he joutuvat nyt lähteä miettimään lisää"). **Alas = "Menkää pois" (Skip):** -1 suosiota kyseiselle ryhmälle, kortti EI kulu — sama vaatimus voi tulla vastaan uudelleen. Tavallinen Ei-hylkäys pysyy omana isompana miinuksenaan (kortin oma suosioluku käännettynä, kuten ennenkin) ja KULUTTAA kortin, koska pelaaja käsitteli vaatimuksen asiallisesti — tämä on tarkoituksellinen ero Skippiin nähden. **Moottorimuutos:** kortti kuluu pakasta vasta kun se kuitataan `hyvaksyAudienssi`/`hylkaaAudienssi`-funktiolla, ei enää heti nostettaessa (`valitseAudienssi` vain "kurkistaa" kortin, `poistaKortti` ajetaan päätöshetkellä). Uudet funktiot `js/moottori/audienssit.js`:ssä: `ehdotaToinenKortti` (palauttaa null jos ei vaihtoehtoa) ja `ohitaAudienssi` (-1 suosiota, ei poista korttia). `hyvaksyAudienssi` sai uuden `ryhmaAvain`-parametrin (yhtenäistetty `hylkaaAudienssi`:n kanssa) jotta se osaa kuluttaa oikean kortin. UI: `js/paa.js`:n `alustaAudienssiSwipe` seuraa nyt sekä X- että Y-akselia (dominoiva akseli ratkaisee diagonaalissa vedossa), uudet napit `ehdota-nappi`/`menkaa-pois-nappi` `index.html`:ssä. Testattu sekä `node --test` (18 uutta/muutettua audienssitestiä) että Playwrightilla selaimessa kaikki neljä suuntaa napeilla ja pointer-eventeillä ajaen — kortin vaihtuminen, toisen yrityksen automaattinen Skip, kummankaan kuluttaman kortin pysyminen pakassa ja tavallisen Kyllä/Ei:n kuluttava käytös kaikki vahvistettu toimiviksi. **GDD:tä ei ole päivitetty** — tämä muistiinpano ja koodi ovat toistaiseksi totuudenlähde, kuten muillekin GDD:n ulkopuolisille päätöksille.
