...
- **Testaus:** `node --test testit/*.test.js` (tai `npm test`) — 133 testiä kattaa kassaraportin, mittarien rajauksen, audienssit, presidentin päätökset, uutisvaiheen, sodan, kriisit, attentaatin, paon, pisteytyksen, kuukausikierroksen vaihejärjestyksen ja poliisiraportin, kaikki vihreää. UI-vuorovaikutus (swipe, vaiheiden napit, paneelien näkyvyys) testataan Playwrightilla selaimessa, ei Node-testeissä.
...
- **Seuraava askel:** koko GDD:n mekaniikkapuoli on toteutettu. Jäljellä on Sasun oma pelitestaus selaimessa sekä kaksi hänelle esitettyä avointa suunnittelukysymystä (ryhmien mittarien näkyvyys ilman raporttia, pisteytystittelin kuvaus kuolemantapauksessa) — ks. muistiinpanot alla.
...
- **AVOIN SASULLE — näkyvätkö ryhmien mittarit ilman poliisiraporttia?** Yläpalkin "Ryhmät"-lista näyttää kaikkien ryhmien suosion ja voiman ilmaiseksi joka hetki. GDD 9.5 ja 12 olettavat päinvastaista: raportti on ainoa tapa nähdä numerot, ja koko puolustusvalinnan jännite ("pelaaja valitsee MUISTINSA varassa", "jos säästit nuo tuhannet, taistelet sokkona") perustuu siihen ettei niitä muuten näe. Nykytoteutuksessa raportin ainoa lisäarvo ovat uhkamerkit, oma voima ja REV STR. Mittarien piilottaminen on yhden funktion muutos (`piirraRyhmat`), mutta se on iso pelituntumamuutos — ei tehty ilman Sasun päätöstä.
